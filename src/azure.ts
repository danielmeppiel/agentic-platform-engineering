...
export class AzureClient {
  ...
  async ensureAuthenticated(): Promise<void> {
    try {
      ...
      if (this.clientId && this.clientSecret && this.tenantId) {
        ...
        throw new Error(
          'No authentication credentials provided. Please provide clientId, clientSecret, and tenantId',
        );
      }
    }
  ...
  async listEnvironmentDefinitions(projectName?: string): Promise<string> {
    ...
    if (!project) {
      throw new Error(
        'Project name must be provided either as a parameter or through DEVCENTER_PROJECT environment variable',
      );
    }
  ...
  async createEntraAppAndSP(params: {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('is already in use')) {
        console.error(
          `Service Principal already exists for app ID: ${appId}, fetching existing SP...`,
        );
    ...
    console.error(
      `Assigning Contributor role to Service Principal for resource group: ${params.deploymentRG}...`,
    );
    ...
    console.error(
      `Successfully assigned Contributor role to resource group: ${params.deploymentRG}`,
    );
  ...
  async createFederatedCredential(params: {
    ...
    if (!appData || !appData.id) {
      throw new Error(
        `Microsoft Entra application with display name "${projectName}-${envType}" not found`,
      );
    }
